...
fn main() {
    ...
    let mut cipher = Aes128Ctr32LE::new(&key.into(), &iv.into());
    // let mut buf:Vec<u8> = blocks.iter().flatten().map(|&x| x).collect()/* .to_vec() */;
    let mut ct:Vec<u8> = blocks.iter().flatten().copied().collect();
    let buf_len = ct.len();
    println!("{buf_len}");
    
    
    cipher.apply_keystream(ct.as_mut_slice());
    ...
    for i in 0..16 {ct[i + 1] = buf_edit[i];}
    cipher.seek(0u32);
    cipher.apply_keystream(ct.as_mut_slice());

    let result_edited_decipd = String::from_utf8(ct).unwrap();
